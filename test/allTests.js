...
const CustomWebSocket = require('../custom-websocket');
...
describe('custom-websocket', function () {
    ...
    it('send text, receive and close with default options', function (done) {
        ...
        const cws = new CustomWebSocket(URL);
    ...
    it('ping (heartbeat)', function (done) {
        ...
        const cws = new CustomWebSocket(URL, null, {heartbeatInterval: 3000});
    ...
    it('reconnect - with shouldReconnect as number', function (done) {
        ...
        const cws = new CustomWebSocket(URL, null, {shouldReconnect: 1500});
        var firstConnection = true;
        ...
        cws.addEventListener('error', function(event) {
            ...
            previousTime = Date.now();
            if (++attempt == 3) { // para simular o servidor voltando
                wss = new WebSocketServer({port: PORT});
            }
        });
    ...
    it('automaticOpen = false, check if connected', function (done) {
        ...
        const anyMessageText = 'hello';
        const wss = new WebSocketServer({port: PORT});
        const cws = new CustomWebSocket(URL, null, {automaticOpen: false});
    ...
    it('no ping (heartbeat = false)', function (done) {
        ...
        const cws = new CustomWebSocket(URL, null, {heartbeat: false, heartbeatInterval: 1000});
    ...
    it('debug = true', function (done) {
        ...
        const cws = new CustomWebSocket(URL, null, {debug: true});
